...
type CartReminderEmailInput = {
  customerEmail: string;
  cartUrl: string;
  items: CartReminderLineItem[];
  subtotal?: string;
};
...
export function buildCartReminderEmail(input: CartReminderEmailInput) {
  const subject = "Your cart is waiting â€” KATSEYE";
  const cartUrl = input.cartUrl;
  const customerEmail = escapeHtml(input.customerEmail);
  const subtotal = input.subtotal ? escapeHtml(input.subtotal) : null;
  ...
  const text = [
    ...
    "",
    ...safeItems.map((item) => `- ${item.name} x${item.quantity} (${item.unitPrice})`),
    subtotal ? `Subtotal: ${input.subtotal}` : null,
    "",
    "Return to your cart:",
    cartUrl,
  ]
    ...
    <div style="max-width:620px;margin:0 auto;padding:36px 18px;">
      <div style="background:#0b0b0a;border:1px solid rgba(240,211,79,0.22);border-radius:18px;overflow:hidden;">
        <div style="padding:22px 22px 16px 22px;background:linear-gradient(90deg,rgba(240,211,79,0.16),rgba(240,211,79,0));border-bottom:1px solid rgba(240,211,79,0.18);">
          ...
          <h1 style="margin:10px 0 0 0;font-size:24px;line-height:1.25;color:#f0d34f;">
            Did you forget something?
          </h1>
          <p style="margin:10px 0 0 0;font-size:14px;line-height:1.6;color:rgba(240,211,79,0.82);">
            Grab the items you left behind before they sell out.
          </p>
        </div>
        ...
        <div style="padding:18px 22px 8px 22px;">
          ...
          ${
            ...
            <div style="font-size:12px;color:rgba(240,211,79,0.7);">Subtotal</div>
            <div style="font-size:18px;font-weight:800;color:#f0d34f;">${subtotal}</div>
          </div>`
              : ""
          }
        </div>
        ...
        <div style="padding:18px 22px 22px 22px;">
          ...
          <p style="margin:14px 0 0 0;font-size:12px;line-height:1.6;color:rgba(240,211,79,0.72);">
            ...
            <a href="${cartUrl}" style="color:#f0d34f;word-break:break-all;">${cartUrl}</a>
          </p>
